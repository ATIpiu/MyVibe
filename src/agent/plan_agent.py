...
# 退出计划模式工具 schema（不通过 ToolRegistry，由 PlanAgent 特殊处理）
EXIT_PLAN_MODE_TOOL = {
    "name": "exit_plan_mode",
    "description": (
        "退出计划模式，返回正常执行模式。"
        "当执行计划已完整制定、用户可以开始执行时调用此工具。"
        "调用后系统会提示用户选择：清空上下文重新执行 / 保持上下文直接执行 / 继续补充计划。"
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {
                "type": "string",
                "description": "计划完成的一句话总结，展示给用户确认",
            }
        },
        "required": [],
    },
...
## 完成计划后
计划完整制定后，调用 exit_plan_mode 工具退出计划模式，让用户决定下一步。
不要在文本里说"计划完成"后等待，直接调用工具。
...
class PlanAgent:
    ...
    def __init__(
        ...
        self._tools.append(EXIT_PLAN_MODE_TOOL)  # 追加退出计划模式工具
        self._plan_dir = Path(cwd) / ".myvibe" / "plans"
        # 计划对话历史（跨多次 run() 调用保持，支持"继续补充"）
        self._messages: list[dict] = []
        # LLM 主动调用 exit_plan_mode 时置 True
    ...
    def _execute_tools(self, tool_calls: list[ToolCall]) -> list[dict]:
        ...
        for tc in tool_calls:
            ...
            # 退出计划模式：特殊处理，不走 ToolRegistry
            if tc.name == "exit_plan_mode":
                summary = tc.input.get("summary", "计划已完成")
                self.exit_requested = True
                self.console.print(f"[bold green][计划] {summary}[/bold green]")
                results.append({
                    "type": "tool_result",
                    "tool_use_id": tc.tool_use_id,
                    "content": "已退出计划模式，等待用户决定下一步。",
                    "is_error": False,
                })
                continue
