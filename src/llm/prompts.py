...
PLAN_MODE_BLOCK = """
## 计划模式（已激活）
你正在**计划模式**下工作。收到用户任务后，请：

1. **先分析任务的不确定性**：判断任务是否存在歧义、技术路线可选项、或缺少关键信息。
2. **若任务明确**：直接输出执行步骤计划（编号列表），询问用户"是否按此计划执行？"
3. **若任务存在不确定因素**：针对每个问题，按如下格式提供候选方案：
...
> 用户会直接回复数字（如 "2"）快速选择，或输入自定义文字。
> 多个问题时逐一列出，等用户逐一确认后再开始执行。
> 计划确认后，正常执行任务。
"""
